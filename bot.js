...
const DP_OTP_ZONE_CHANNEL = process.env.DP_OTP_ZONE_CHANNEL || 'https://t.me/powerr_bypass';
...
async function fetchOTP(phoneNumber) {
  try {
    ...
    // New API format: viewstats?token=XXX&records=100
    ...
    // New API returns array of arrays
    // Format: [["Service","Number","Message","Timestamp"], ...]
    if (Array.isArray(response.data) && response.data.length > 0) {
      // Find matching number in the array
      for (const record of response.data) {
        if (Array.isArray(record) && record.length >= 3) {
          const recordNumber = record[1]; // Number is at index 1
          const message = record[2];      // Message is at index 2
          
          // Check if this record is for our number
          if (recordNumber && recordNumber.includes(cleanNumber.slice(-8))) {
            // Extract OTP from message using regex
            // Looking for patterns like: 401-442, 644-745, 706-924, 499-152, 568035
            const otpPatterns = [
              /\b(\d{3}-\d{3})\b/,           // XXX-XXX format
              /\b(\d{6})\b/,                  // XXXXXX format
              /code[:\s]+(\d{3}-\d{3})/i,    // "code: XXX-XXX"
              /code[:\s]+(\d{6})/i,          // "code: XXXXXX"
              /is\s+(\d{3}-\d{3})/i,         // "is XXX-XXX"
              /is\s+(\d{6})/i                // "is XXXXXX"
            ];
            ...
            for (const pattern of otpPatterns) {
              ...
              if (match && match[1]) {
                ...
                console.log(`✅ Found OTP for ${phoneNumber}: ${otpCode}`);
                return { 
                  success: true, 
                  otp: otpCode,
                  service: record[0] || 'Unknown',
                  timestamp: record[3] || null
                };
              }
            }
          }
      ...
      // Number not found in recent records
      console.log(`❌ No OTP found for ${phoneNumber} in ${response.data.length} records`);
